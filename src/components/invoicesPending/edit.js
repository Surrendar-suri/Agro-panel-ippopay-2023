...
import { PlusCircleOutlined, LoadingOutlined, PlusCircleFilled, DeleteOutlined } from "@ant-design/icons";
...
import { isInvalidName } from '../../helpers/Utils';
import {
    createInvoice,
    updateInvoice,
    invoiceDetails,
...
import { invoiceItemList, invoiceItemDetails, updateItem, createInvoiceItem } from '../../store/actions/invoiceItem';
...
function AddEdit(props) {
    ...
    const [customerAddress, setCustomerAddress] = useState(params.id ? true : false)
    ...
    const [adjustmentrate, setAdjustmentRate] = useState(0.00);
    ...
    const farmerLIST = () => {
        ...
        dispatch(myprofile((result) => {
            if (result) {
                ...
                setState({ ...state, terms_condition: data?.settings?.invoice?.terms ?? "" })
    ...
    const farmerLIST2 = () => {
        ...
        dispatch(farmerActiveList((result) => {
            if (result) {
    ...
    const farmerLIST3 = () => {
        ...
        dispatch(businessActiveList((result) => {
            if (result) {
    ...
    const ItemList = () => {
        let query = '';
        dispatch(invoiceItemList(query, (result) => {
            if (result) {
        ...
        dispatch(
            invoiceDetails(params.id, (result) => {
                if (result) {
                    ...
                    setFarmer_f2m(data.invoice_type === "f2m"
                        ? data.invoice_type
                        : setFarmerCheck(false));
                    setBusiness_m2b(data.invoice_type === "m2b"
                        ? data.invoice_type
                        : setBusinessCheck(false))
                    setBusinessAddName(business ? data?.customer?.name ?? "" : "");
                    setbusiness_bill_add_1(business ? customer?.billing?.line_1 ?? "" : "");
                    setbusiness_bill_add_2(business ? customer?.billing?.line_2 ?? "" : "");
                    setbusiness_bill_city(business ? customer?.billing?.city?.name ?? "" : "");
                    setbusiness_bill_state(business ? customer?.billing?.state?.name ?? "" : "");
                    setbusiness_bill_zipcode(business ? customer?.billing?.zipcode ?? "" : "");
                    setbusinessAddress(business ? customer : "");
                    setbusiness_ship_add_1(business ? customer?.shipping?.line_1 ?? "" : "");
                    setbusiness_ship_add_2(business ? customer?.shipping?.line_2 ?? "" : "");
                    setbusiness_ship_city(business ? customer?.shipping?.city?.name ?? "" : "");
                    setbusiness_ship_state(business ? customer?.shipping?.state?.name ?? "" : "");
                    setbusiness_ship_zipcode(business ? customer?.shipping?.zipcode ?? "" : "");
                    setbusiness_same_billing(business ? customer?.is_same_billing ?? "" : "");
                    ...
                    setfarmer_bill_city(farmer ? customer?.billing?.city?.name ?? "" : "");
                    setfarmer_bill_state(farmer ? customer?.billing?.state?.name ?? "" : "");
                    setfarmer_bill_zipcode(farmer ? customer?.billing?.zipcode ?? "" : "");
                    setfarmerAddress(farmer ? customer : "");
                    setFarmerCheck(farmer ? true : false);
                    setBusinessCheck(business ? true : false);
                    setCustomer(
                        data?.customer?.customer_type === "Business" ? data?.customer?.name ?? "" : ""
                    );
                    setFarmer(
                        data?.customer?.customer_type === "Farmer" ? data?.customer?.name ?? "" : ""
                    );
                    ...
                    setcheckedPartial(
                        data?.partial_payment?.is_allowed ?? ""
                    );
    ...
    const cancelPass = () => {
        setmodalItem({ ...modalItem, name: "", description: "", cost: "", currency: "", quantity: "", item_id: "", gst: "" })
    ...
    const createSignup = () => {
        ...
        // }
            ...
            if (modalItem.item_id) {
                dispatch(updateItem(data, modalItem.item_id, (result) => {
                    if (result) {
                        setModal2Visible(false);
                        setmodalItem({ ...modalItem, item_id: "", name: "", description: "", cost: "", currency: "", quantity: "", gst: "" })
                        ItemList();
                    }
                }))
            } else {
                dispatch(createInvoiceItem(data, (result) => {
                    if (result) {
                        setModal2Visible(false);
                        setmodalItem({
                            ...modalItem, name: "", description: "", cost: "", currency: "", quantity: "", gst: ""
                        })
                    }
                }))
            }
    ...
    const handlePriceChange = (e, index) => {
        ...
        const { name, value } = e.target;
        const list = [...inputList];
        list[index][name] = isNaN(value) || value < 0 || value === ""
            ? ""
            : parseInt(value);
        setInputList(list);
        getsubTotal();
    ...
    const getsubTotal = () => {
        ...
        inputList.forEach((item) => {
            ...
            let total = (item.cost * item.qty) + ((item.cost * item.qty) * item.gst / 100);
            subtotal += total;
        });
    ...
    useEffect(() => {
        ...
        if (discount === INVOICE_TYPES.CURRENCY) {
            ...
            setTotalAmount((subTotal - discountPrice + taxes) + Number(adjustmentPrice));
        } else {
            setRate((subTotal * discountPrice) / 100);
            setAdjustmentRate(adjustmentPrice)
            setTotalAmount((subTotal - (subTotal * discountPrice) / 100 + taxes) + Number(adjustmentPrice));
        }
    ...
    const createInvoiceItems = (item) => {
        ...
        if (heading === "" || heading === null) {
        ...
        } else if (business_m2b === "m2b" && (role === "" || role === null || role === undefined)) {
            toastError("Please select the business owner name");
        } else if (farmer_f2m === "f2m" && (role === "" || role === null || role === undefined)) {
        ...
        } else if (business_m2b === "m2b" && checkedPartial && (payment_select === "" || payment_select === null || payment_select === undefined)) {
            toastError("Please select the partial counts");
        } else if (totalAmount < 0.90) {
            ...
            let data = {
                ...
                tax: Tax,
                customer: {
                    id: business_m2b === "m2b" ? selectedBusiness.business_id : selectedFarmer.farmer_id,
                    customer_type: business_m2b === "m2b" ? "business" : "farmer",
                    name: business_m2b === "m2b" ? selectedBusiness?.name?.full : selectedFarmer?.name?.full,
                },
                merchant: {
                    id: merchant,
                },
                ...
                dispatch(
                    updateInvoice(data, state.id, (result) => {
                        if (result) {
                            history.push("/invoices-pending");
                            if (create === "preview") {
                                history.push("/invoices-pending/details/" + state.id);
                            } else if (create === "draft") {
                                history.push("/invoices-pending/edit/" + state.id);
                            }
                        }
                    })
                );
    ...
    const merchantBillingAddress = (
        <span className="bill_add_deatils">
            {merchant_billing_city !== "" && merchant_billing_zipcode !== ""
                ? merchant_billing_city + " - " + merchant_billing_zipcode
                : merchant_billing_city !== ""
                    ? merchant_billing_city ? merchant_billing_city : ""
                    : merchant_billing_zipcode ? merchant_billing_zipcode : ""}
        </span>
    );
    ...
    const farmerFlat = (
        <span className="bill_add_deatils">
            {farmer_bill_add_1 !== "" && farmer_bill_add_2 !== ""
                ? farmer_bill_add_1 + " , " + farmer_bill_add_2
                : farmer_bill_add_1 !== ""
                    ? farmer_bill_add_1 ? farmer_bill_add_1 : ""
                    : farmer_bill_add_2 ? farmer_bill_add_2 : ""}
        </span>
    );
    ...
    const businessBillingFlat = (
        <span className="bill_add_deatils">
            {business_bill_add_1 !== "" && business_bill_add_2 !== ""
                ? business_bill_add_1 + " , " + business_bill_add_2
                : business_bill_add_1 !== ""
                    ? business_bill_add_1 ? business_bill_add_1 : ""
                    : business_bill_add_2 ? business_bill_add_2 : ""}
        </span>
    );
    ...
    const businessShippingFlat = (
        <span className="bill_add_deatils">
            {business_ship_add_1 !== "" && business_ship_add_2 !== ""
                ? business_ship_add_1 + " , " + business_ship_add_2
                : business_ship_add_1 !== ""
                    ? business_ship_add_1 ? business_ship_add_1 : ""
                    : business_ship_add_2 ? business_ship_add_2 : ""}
        </span>
    );
    ...
    const businessShippingAddress = (
        <span className="bill_add_deatils">
            {business_ship_city !== "" && business_ship_zipcode !== ""
                ? business_ship_city + " - " + business_ship_zipcode
                : business_ship_city !== ""
                    ? business_ship_city ? business_ship_city : ""
                    : business_ship_zipcode ? business_ship_zipcode : ""}
        </span>
    )
    ...
    const businessBillingAddress = (
        <span className="bill_add_deatils">
            {business_bill_city !== "" && business_bill_zipcode !== ""
                ? business_bill_city + " - " + business_bill_zipcode
                : business_bill_city !== ""
                    ? business_bill_city ? business_bill_city : ""
                    : business_bill_zipcode ? business_bill_zipcode : ""}
        </span>
    )
    ...
    const businessNameChange = (id) => {
        dispatch(bussinessDetails(id, (result) => {
            if (result) {
    ...
    const uploadImages = async (event) => {
        ...
        if (file.type === "image/png" || file.type === "image/jpg" || file.type === "image/jpeg") {
            ...
            if (file.size > 5e6) {
                ...
                dispatch(UploadProfile(merchant, upload, formData, (result) => {
                    if (result) {
                        setTimeout(() => {
                            setImageLoaded(false)
    ...
    const listOnclick = (value) => {
        ...
        const list = [...inputList];
        list[inputIndex] = { ...list[inputIndex], name: value.name, cost: value.cost, desc: value.description, gst: value.gst }
        setInputList(list);
        setItemSelect(false)
        setInputIndex("")
                        ...
                        <div className="container-fluid main-container p-0 ">
                            <>
                                ...
                                <div className="invoice_card_2 agro_card">
                                    <div className="container_top" style={{ width: "68%", margin: "0" }}>
                                        <div
                                        ...
                                        >
                                            <>
                                                ...
                                                >
                                                    <div className="col-xs-12  brd_bottom" style={{ padding: "0px 0px 20px 0" }}>
                                                        <div className="col-xs-7 ">
                                                            ...
                                                            <div>
                                                                <div className="bill_left">
                                                                    ...
                                                                    {merchantBillingAddress && merchantBillingAddress}
                                                                    <span className="bill_add_deatils">
                                                                        {merchantAddress?.merchant_billing_state ?? ""}
                                                                    </span>
                                                                    {merchantAddress.is_same_merchant_billing === false && (
                                                                        <>
                                                                            <span className="bill_add_deatils" style={{ fontWeight: "bold", marginTop: 15 }}>
                                                                                Shipping
                                                                            </span>

                                                                            {merchantAddress?.merchant_shipping_add_line1 ?? ""}
                                                                            {merchantShippingAddress && merchantShippingAddress}
                                                                            <span className="bill_add_deatils">
                                                                    ...
                                                                    {checked && merchantAddress?.gst_number !== "" && (
                                                                        <span className="bill_add_deatils">GST - {merchantAddress?.gst_number ?? ""}</span>
                                                                ...
                                                                <>
                                                                    ...
                                                                    {(merchantImage !== "" && merchantImage !== undefined && merchantImage !== null) && (
                                                                        <div className="logo_upload_merchant">
                                                                            ...
                                                                            <img className='pop-body-vi' src={merchantImage} alt="upload img" accept='image/*' />
                                                                    ...
                                                                    {(merchantImage === "" || merchantImage === undefined || merchantImage === null) && (
                                                                        <div className="logo_upload" onChange={uploadImages}>
                                                                            ...
                                                                            <img className='' style={{ display: "block", margin: "0 auto" }} src={uploadImage} alt="upload img" />
                                                        ...
                                                        <div className="col-xs-6 ">
                                                            <div className="add_input m-0">
                                                                <div className="add_input ">
                                                                    ...
                                                                    />
                                                                    <label className="label_margin ivc_type_name" for="Farmer">
                                                                        Farmer
                                                                    </label>
                                                                    <input
                                                                        type="radio"
                                                                    ...
                                                                    />
                                                                    <label className="label_margin ivc_type_name" for="Business">
                                                                        Business
                                                                    </label>
                                                                </div>
                                                                ...
                                                                {customerAddress ? (
                                                                    ...
                                                                    <div className="col-xs-6" style={{ width: "107%", display: "flex" }}>
                                                                        <div className="customer-border">

                                                                            <div className="customer_address" style={{ marginTop: 15 }}>
                                                                                {(farmer_f2m === "f2m" && farmerAddress) ? (
                                                                            ...
                                                                            <div className="customer_address" style={{ marginTop: 15 }}>
                                                                                ...
                                                                                {(business_m2b === "m2b" && businessAddress) ? (
                                                                                    ...
                                                                                    <>
                                                                                        ...
                                                                                        {businessBillingFlat && businessBillingFlat}
                                                                            ...
                                                                            <div className="customer_address" style={{ marginTop: 15 }}>
                                                                                {(business_m2b === "m2b" && businessAddress && !business_same_billing) ? (
                                                                                    <>
                                                                                        <span className="bill_add_deatils" style={{ fontWeight: "bold" }}>
                                                                                            Shipping To
                                                                                        </span>

                                                                                        {businessShippingFlat && businessShippingFlat}


                                                                                        {businessShippingAddress && businessShippingAddress}
                                                                                        <span className="bill_add_deatils">
                                                                                            {business_ship_state}
                                                                                        </span>
                                                                                    </>
                                                                            ...
                                                                            <div>
                                                                                <p onClick={customerToggle} className="differ_cus">Choose a different customer</p>
                                                                            </div>
                                                                        </div>
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        {farmer_f2m === "f2m" && state.select_farmer_dash ? (
                                                                            <div className="img_upload_sec_customer" onClick={() => setState({ ...state, select_farmer_dash: false })}>
                                                                                <span className="customer_dash">SELECT A FARMER</span>
                                                                            </div>
                                                                        ) : farmer_f2m === "f2m" && (
                                                                            <div className="add-input">
                                                                                ...
                                                                                <Select
                                                                                    ...
                                                                                    showSearch
                                                                                    style={{ width: "100%" }}
                                                                                    value={defaultRole ? defaultRole.name : "SELECT A FARMER"}
                                                                                    autoFocus={true}
                                                                                    ...
                                                                                    onChange={(value) => {
                                                                                        ...
                                                                                        farmerNameChange(JSON.parse(value).id);
                                                                        ...
                                                                        {business_m2b === "m2b" && state.select_business_dash ? (
                                                                            <div className="img_upload_sec_customer" onClick={() => setState({ ...state, select_business_dash: false })}>
                                                                                <span className="customer_dash">SELECT A BUSINESS</span>
                                                                            </div>
                                                                        ) : business_m2b === "m2b" && (
                                                                            <div className="add-input m-0">
                                                                                ...
                                                                                <Select
                                                                                    ...
                                                                                    filterOption={(input, option) => option.children.toLowerCase().includes(input.toLowerCase())}
                                                                                    name="customer"
                                                                                    value={defaultRole ? defaultRole.name : "SELECT A BUSINESS"}
                                                                                    onChange={(value) => {
                                                                                        ...
                                                                                        setRole(value)
                                                                                        setCustomerAddress(true)
                                                                                        businessNameChange(JSON.parse(value).id);
                                                                                    }}
                                                                                >
                                                                                    {businessess &&
                                                            ...
                                                            <thead>
                                                                <tr className="table_head">
                                                                    ...
                                                                    <th className="text-right" >
                                                                        <span style={{ justifyContent: 'end', paddingRight: "15px" }} >Amount</span>
                                                                    </th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                {item && item.length !== 0 ? item && item.map((items, i) => (
                                                                    <tr key={i} className="invoicechild_tr">
                                                                            ...
                                                                            <span>
                                                                                {inputList.length - 1 !== i ? (
                                                                                    ...
                                                                                    <p style={{ margin: 0 }}>
                                                                                        ...
                                                                                        <span className="add-field" onClick={handleAddClick}>
                                                                                            <PlusCircleOutlined />
                                                                                        </span>
                                                                                    </p>
                                                                                )}
                                                                        ...
                                                                        <td style={{
                                                                            ...
                                                                            <span>
                                                                                ...
                                                                                />{(inputIndex === i) && (
                                                                                    <div className="item-list-box2" style={{
                                                                                        top: "85%",
                                                                                        left: "10px",
                                                                                        width: "217px"
                                                                                    }}>
                                                                                        {itemList && itemList.filter((val) => {

                                                                                            const list = [...inputList];
                                                                                            const searchTerm = list[inputIndex].name.toLowerCase();
                                                                                            const item_name = val.name.toLowerCase();

                                                                                            return searchTerm && item_name.startsWith(searchTerm)
                                                                                            ...
                                                                                            return (
                                                                                                ...
                                                                                                <ul className="drop_list" >
                                                                                                    <li key={j} onClick={() => listOnclick(item)}>
                                                                        ...
                                                                        <td>
                                                                            <span>
                                                                                <Input
                                                                                    ...
                                                                                    style={{ height: "35px", borderRadius: "5px" }}
                                                                                    autoComplete="off"
                                                                                    onChange={(e) => handleInputDesChange(e, i)}
                                                                        ...
                                                                        <td>
                                                                            <span>
                                                                                <Input
                                                                                    name="cost"
                                                                                    style={{ width: "90%", textAlign: "right", height: "35px", borderRadius: "5px", backgroundColor: 'transparent' }}
                                                                                    value={items.cost}
                                                                        ...
                                                                        <td>
                                                                            <span>
                                                                                <Input
                                                                                    name="qty"
                                                                                    style={{ width: "70px", textAlign: "right", height: "35px", borderRadius: "5px" }}
                                                                                    value={items.qty}
                                                                                    placeholder="0"
                                                                                    autoComplete="off"
                                                                                    ref={inputRef}
                                                                            ...
                                                                            <span className="discount_con">
                                                                                <div className="modal_amount gst_input">
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        <span
                                                                                            style={{ width: "25px", display: "flex", justifyContent: "center", alignItems: "center", background: "#eee" }}
                                                                                            className="discount"
                                                                                        >
                                                                                            %
                                                                                        </span>
                                                                        ...
                                                                        <td>
                                                                            <span className="price_col" style={{ paddingRight: "15px" }}>
                                                                                &#x20B9; {toFixed((items.cost * items.qty) + ((items.cost * items.qty) * items.gst / 100))}
                                                                            </span>
                                                                        </td>
                                                                    </tr>
                                                                )) : null}
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td colSpan={3} >
                                                                        <span>
                                                                            <div>
                                                                                <div className="inv" onClick={() => setModal2Visible(true)}>
                                                                                    {" "}
                                                                ...
                                                                <tr className="invoicechild_tr">
                                                                    <td colSpan={6} style={{ textAlign: 'right' }}><span className="subtotal_label">Subtotal</span></td>
                                                                    <td className="table-float"> <span className="price_col" style={{ paddingRight: "15px" }}>&#x20B9; {toFixed(subTotal)}</span></td>
                                                                </tr>
                                                                <tr className="invoicechild_tr">
                                                                    <td colSpan={6} style={{ textAlign: 'right' }}>
                                                                        <span className="discount_con">
                                                                            <label className="discount_label">Discount</label>
                                                                            ...
                                                                            {item.is_enabled && (
                                                                                <tr className="invoicechild_tr">
                                                                                    ...
                                                                                    <td className="table-float">
                                                                                        <span className="price_col" style={{ paddingRight: "15px" }}>
                                                                                            &#x20B9;{" "}
                                                                                            {toFixed(item.value ? item.value : 0)}
                                                                                        </span>
                                                                                    </td>
                                                                ...
                                                                <tr className="invoicechild_tr">
                                                                    <td colSpan={6} style={{ textAlign: 'right' }}>
                                                                        <span className="discount_con">
                                                                            <label className="discount_label">Adjustment</label>
                                                                            <div className="modal_amount">
                                                                                <Input.Group
                                                                                    compact
                                                                                    style={{
                                                                ...
                                                                <tr className="invoicechild_tr">
                                                                    <td colSpan={6} style={{ textAlign: 'right' }}><span className="totalamt_label">Total Amount</span></td>
                                                                    <td className="table-float">
                                                                        <span className="price_col " style={{ paddingRight: "15px" }}>
                                                                            &#x20B9; {toFixed(totalAmount)}
                                                                        </span>
                                                ...
                                                <div className="add_column">
                                                    ...
                                                    <div className="terms_head">
                                                        <span className="terms_title">TERMS AND CONDITIONS</span>
                                                        <TextArea
                                                        ...
                                                        <p className="future_check">
                                                            <input type="checkbox" name="futures_condition" checked={checkFuture} onChange={onChangeFuture} />
                                    ...
                                    <div className="invoice_right" style={{ width: "32%" }}>
                                        <div className="rightsec_fix">
                                            {businessArray?.invoice_type === "f2m" || farmer_f2m === "f2m" ? "" : (
                                                <div >
                                                    {businessCheck && (
                                                        <Card className="invoice_card" style={{ margin: 0 }}>
                                                            <div className="invoice_card_1">
                                                                <div>
                                                                    <h4 className="tab_head" style={{ margin: 0 }}>
                                                    ...
                                                    <Card className="invoice_card " style={{ marginTop: businessCheck ? "3px" : "0px" }}>
                                                        <div className="invoice_card_1">
                                                            <div>
                                                                <h4 className="tab_head" style={{ marginBottom: "15px" }}>
                                                                    Enable Partial Payments
                                                                </h4>

                                                                <div className={checkedPartial ? "checkPartial" : "HidecheckPartial"}>
                                                                    <h5 className="tab_des">Allow accepting payments in parts</h5>
                                                                    <Select style={{ width: "210px" }} name="payment_select" value={state.payment_select || undefined} onChange={payment_drop} disabled={checkedPartial ? false : true} placeholder="Selected By">
                                            ...
                                            <Card className={businessArray?.invoice_type === "f2m" || farmer_f2m === "f2m" ? "invoice_card_top" : "invoice_card "} >
                                                ...
                                                (businessArray?.invoice_type === "f2m" || farmer_f2m === "f2m" ? (
                                                    ...
                                                    <Card className="invoice_card">
                                                        ...
                                                        <div className="send-invoice">
                                                            ...
                                                            <div className="invoice_card_3">
                                                                <button className="creates_btn">Send</button>
                                                                <button className="create_share">
                                                                    Get Share Link
                                                                </button>
                                                                <button className="creates_btn">Download</button>
                                                            </div>
                            ...
                            <div>
                                ...
                                >
                                    <Form.Item
                                        label="Name"
                                    >
                                        <Input name="name" placeholder="Name" value={modalItem.name} onChange={handleItemChange} />
                                    </Form.Item>
                                    {/* <Form.Item
                                            label="Quantity"
                                        >
                                    ...
                                    <Form.Item
                                        label="Description"
                                    >
                                        <Input name="description" placeholder="description" value={modalItem.description} onChange={handleItemChange} />
                                    </Form.Item>
                                    <Form.Item
                                        label="Cost"
                                    >
                                        <Input name="cost" placeholder="cost" value={modalItem.cost} onChange={handleItemChangeNumber} />
                                    </Form.Item>
                                    <Form.Item
                                        label="GST"
                                    >
                                        <Input name="gst" placeholder="Gst" value={modalItem.gst} onChange={handleItemChangeNumber} />
                                    </Form.Item>
